...
def prepare_datasets(data_dir):
    ...
    if has_subfolders_with_images:
        ...
        if not files:
            raise FileNotFoundError(f"No image files found under {data_dir}. Supported extensions: {sorted(IMG_EXTS)}")
...
def build_model(num_classes, device):
    ...
    try:
        ...
        log(f"Could not load pretrained weights. Falling back to non-pretrained. Reason: {e}")
...
def main():
    ...
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=WORKERS, pin_memory=True) if val_ds is not None else None
    ...
    with torch.no_grad():
        ...
        log(f"Sample predictions (first batch):")
        for i in range(min(6, sample_x.size(0))):
            top1 = sample_logits[i].argmax().item()
            confidence = float(sample_logits[i, top1].item())
            log(f"  index {i} -> class: {class_names[top1]}  conf: {confidence:.3f}")
